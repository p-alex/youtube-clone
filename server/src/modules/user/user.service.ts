...
export const searchChannels = async ({ query }: { query: string }) => {
  const limit = 15;
  const result = await db.query(
    "SELECT user_id, username, profile_picture, description, total_videos, total_subscribers FROM users WHERE username LIKE $1 ORDER BY total_subscribers DESC LIMIT $2",
    [`%${query}%`, limit]
  );
